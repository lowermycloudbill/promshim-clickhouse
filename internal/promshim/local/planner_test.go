...
func TestLocalSubqueryPlanRangeModeNoStepUsesDefaultEvaluationInterval(t *testing.T) {
	...
	// Left-open window (t-range, t]: the point at Start-range (0) is excluded
	// (subquery-left-open-window fix), so the 60s default grid yields 20 points
	// strictly after 0 (60..1200) rather than 21.
	if len(calls) != 20 {
...
func TestLocalSubqueryPlanUsesConfiguredDefaultEvaluationInterval(t *testing.T) {
	...
	t.Run("no_step_uses_configured_interval", func(t *testing.T) {
		...
		// Left-open window (t-range, t]: the point at t-range (0) is excluded
		// (subquery-left-open-window fix); the 30s configured interval then
		// yields 30-spaced grid points strictly after 0.
	...
	t.Run("explicit_step_wins_over_configured_interval", func(t *testing.T) {
		...
		// Left-open window (t-range, t]: the point at t-range (0) is excluded
		// (subquery-left-open-window fix); the explicit 1m step yields
		// 60-spaced grid points strictly after 0.
		want := []int64{60, 120}
		if len(calls) != len(want) {
			t.Fatalf("expected %d child evaluations, got %d (%v)", len(want), len(calls), calls)
		}
