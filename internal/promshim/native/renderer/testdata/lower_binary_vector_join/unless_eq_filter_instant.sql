...
)) AS lhs LEFT JOIN (SELECT join_group, toUInt8(1) AS present_marker FROM (SELECT tags AS original_group, arraySort(tag -> tag.1, arrayFilter(tag -> NOT has(['__name__'], tag.1), tags)) AS join_group, timestamp AS timestamp, value AS value FROM (
