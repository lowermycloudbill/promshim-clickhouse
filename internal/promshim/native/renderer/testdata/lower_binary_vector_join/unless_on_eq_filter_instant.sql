...
SELECT result_tags AS tags, timestamp AS timestamp, value AS value FROM (SELECT lhs.original_group AS result_tags, lhs.timestamp AS timestamp, lhs.value AS value FROM (SELECT tags AS original_group, arraySort(tag -> tag.1, arrayFilter(tag -> has(['instance'], tag.1), tags)) AS join_group, timestamp AS timestamp, value AS value FROM (
    SELECT tags AS tags, timestamp AS timestamp, value AS value FROM cse_selector_3ea73066c075
)) AS lhs LEFT JOIN (SELECT join_group, toUInt8(1) AS present_marker FROM (SELECT tags AS original_group, arraySort(tag -> tag.1, arrayFilter(tag -> has(['instance'], tag.1), tags)) AS join_group, timestamp AS timestamp, value AS value FROM (
    SELECT tags AS tags, timestamp AS timestamp, value AS value FROM (
        SELECT tags AS tags, timestamp AS timestamp, value AS value FROM cse_selector_3ea73066c075
    ) WHERE (value) = (0)
)) AS rhs GROUP BY join_group) AS rhs ON lhs.join_group = rhs.join_group WHERE ifNull(rhs.present_marker, 0) = 0) AS joined_rows ORDER BY result_tags
